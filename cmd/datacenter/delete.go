...
func DeleteCommand() *cli.Command {
	return &cli.Command{
		...
		Description: "Delete a datacenter from the inventory",
		...
		Run: func(ctx context.Context, cmd *cli.Command) error {
			...
			log.Debug("Deleting datacenter", "id", id, "server", cmd.GetString("server"))
			
			client := &http.Client{Timeout: 30 * time.Second}
			req, err := http.NewRequest("DELETE", cmd.GetString("server")+"/api/datacenters/"+id, nil)
			if err != nil {
			...
			if resp.StatusCode == http.StatusNotFound {
				log.Warn("Datacenter not found for deletion", "id", id)
			...
			fmt.Println("Datacenter deleted")
			return nil
		},
	}
}